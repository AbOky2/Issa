...
export default ({ onClick, text = '', redirect = false, href = null, full = true, auto = true, grayColor = false, greenColor = false, opaqueGreenColor }) => {
    let className = 'btn-container '
    let fields = { full, auto, grayColor, greenColor, opaqueGreenColor },
        elem;

    className += Object.keys(fields).filter(e => fields[e] == true).join(' ')
    if (href)
